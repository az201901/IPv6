...
* ルータからホストへ、より適切な送出先を伝える方法（ Redirect )
...
を、定期的にマルチキャストで送信する。サブネットに接続して
いる機器は、RA を受け取ることにより、そのサブネットで通信を行うための情報を得る。

　 Router Solicitation を送ることにより、 Router Advertisement を直ちに受け取ることも可能である。

## 7.2.1　Router Advertisement メッセージ

　RA は、タイプ 134 の ICMPv6 メッセージとなる。

　RA パケットの、送信元 IPv6 アドレスには、
送信に使われるインターフェースに設定されたリンクローカルアドレスが設定される。

　宛先 IPv6 アドレスには、通常は全ノードマルチキャストアドレス（ ff02::1 ）が設定される。
RS に対する返答 RA の場合は、 RS の送信元 IPv6 アドレスが設定される。

　RA パケットでは、Hop Limit の値に 255 が設定される。これにより、他のリンクに接続されたルータから RA が
転送されてきても、その内容に影響されないようにできるようになっている。
